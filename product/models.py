...
class Category(DateMixin):
    ...
    def clean(self):
        ...
        if self.parent_name and self.parent_name.is_descendant_of(self):
            raise ValidationError(_("Invalid parent assignment to prevent recursion."))
...
class ProductVideoType(DateMixin):
    ...
    cover_image_url = models.URLField(_("Cover Image"), max_length=255, null=True, blank=True)
...
class UserProductLike(DateMixin):
    user = models.OneToOneField(User, verbose_name=_("User"), on_delete=models.CASCADE, related_name='user_like')
...
class ProductComment(DateMixin):
    ...
    parent_comment = models.ForeignKey("ProductComment", verbose_name=_("Parent Comment"), on_delete=models.CASCADE, related_name='comment_child', null=True, blank=True)
...
class ProductCommentLike(DateMixin):
    user = models.OneToOneField(User, verbose_name=_("User"), on_delete=models.CASCADE, related_name='user_comment_like')
