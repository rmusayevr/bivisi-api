...
class WishlistAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        message = {'success': False,
                   'message': 'You have to log in to add product your wishlist!'}
...
class BasketAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        product = Product.objects.filter(pk=product_id).first()
        if product:
            for_basket, created = BasketItem.objects.get_or_create(
                product=product, user=request.user)
            basket2 = BasketItem.objects.get(
                product=product, user=request.user)
            basket2.quantity += quantity
            basket2.save()
            basket1, created = Basket.objects.get_or_create(
                user=request.user, is_active=True)
            ...
            data["message"] = {
                'success': True, 'message': 'Product was added to basket!', 'items': arr}
            message = {'success': True,
                       'message': 'Product added to your card.'}
            return Response(data, status=status.HTTP_201_CREATED)
        message = {'success': False,
                   'message': 'You have to log in to add product your basket!'}
        return Response(message, status=status.HTTP_401_UNAUTHORIZED)

    def delete(self, request, *args, **kwargs):
        ProductID = request.data.get('product')
        if ProductID:
            user_basket = BasketItem.objects.get(
                product=ProductID, user=request.user)
            user_basket.delete()
